...
class EditPhoneViewController: BaseViewController {
  ...
  func bindInput() {
    ...
    confirmButton.rx.tapGesture().when(.recognized)
      .bind(onNext: { [weak self] _ in
        ...
        let param = PatchUserInfoRequest(phoneToken: phoneToken)
        self.showHUD()
...
//        self.navigationController?.pushViewController(vc, animated: true)
  ...
  func sendAuthCode() {
    ...
    showHUD()
    let param = SendAuthCodeRequest(phone: phoneTextField.text!, type: .changePhone)
    APIService.shared.authAPI.rx.request(.sendAuthCode(param: param))
      ...
      .subscribe(onSuccess: { response in
        ...
        if error.serverMessage == "already_in_use" {
          self.callMSGDialog(message: "이미 사용중인 휴대폰 번호입니다")
        } else {
  ...
  func confirmAuthCode() {
    ...
    APIService.shared.authAPI.rx.request(.confirmAuthCode(param: param))
      ...
      .subscribe(onSuccess: { response in
        ...
        self.confirmButtonLabel.textColor = .white
      }, onFailure: { error in
        self.dismissHUD()
